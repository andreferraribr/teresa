...
```{r}
# 
# saldo_trabalho <- dados_servidores %>% filter(data %in% c(max(data), add_with_rollback(max(data), months(-1))))%>% group_by(regime,cargo_clean,mes_ano) %>% count() %>% pivot_wider(names_from = "mes_ano", values_from = n) 
# 
